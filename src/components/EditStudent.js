...
function EditStudent() {
    ...
    useEffect(() => {
        Axios.get("https://crud-deployment-backend-k1yb.onrender.com/studentRoute/update-student/" + id)
            .then((res) => {
                if (res.status === 200) {
    ...
    return (
        <form onSubmit={handleSubmit}>
            <StudentForm getState={getState} nameValue={data.name} emailValue={data.email} rollNoValue={data.rollNO} >Update Student</StudentForm>
        </form>
    )
}
